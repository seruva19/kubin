...
from train_modules.lora_22.train_lora_prior import (
    default_lora_prior_config_path,
    add_default_values,
    launch_lora_prior_training,
)


def train_lora_prior_ui(kubin, tabs):
    default_lora_config = load_config_from_path(default_lora_prior_config_path)
    default_lora_config = add_default_values(default_lora_config)

    with gr.Row() as train_lora_prior_block:
        current_lora_config = gr.State(default_lora_config)

        with gr.Column(scale=3):
            with gr.Accordion("Paths", open=True) as paths_ui:
                with gr.Row():
                    pretrained_prior_path = gr.Textbox(
                        value=default_lora_config["paths"]["pretrained_prior_path"],
                        label="Pretrained prior path",
                        info=text_tip(
                            "Path to pretrained prior model or model identifier from huggingface"
                        ),
                    )
                ...
                with gr.Row():
                    image_processor_path = gr.Textbox(
                        ...
                        info=text_tip("Path to image processor"),
                    )
                    text_encoder_path = gr.Textbox(
                        value=default_lora_config["paths"]["text_encoder_path"],
                        label="Text encoder path",
                        info=text_tip("Path to text encoder"),
                    )
                    tokenizer_path = gr.Textbox(
                        value=default_lora_config["paths"]["tokenizer_path"],
                        label="Tokenizer path",
                        info=text_tip("Path to tokenizer"),
                    )
            ...
            with gr.Accordion("Dataset", open=True) as dataset_ui:
                with gr.Row():
                    train_images_paths_csv = gr.Textbox(
                        ...
                        label="Train images paths (CSV)",
                        info=text_tip(
                            "Path to csv with train images paths with column paths and caption"
                        ),
                    )
                    ...
                    val_images_paths_csv = gr.Textbox(
                        ...
                        info=text_tip(
                            "Path to csv with validation images paths with column paths and caption"
                        ),
                        interactive=False,
                    )
            ...
            with gr.Accordion("Training", open=True) as training_ui:
                ...
                with gr.Row():
                    copy_training_params = gr.Button(
                        "📑 Copy training params from decoder tab",
                        size="sm",
                        interactive=False,
                    )
            ...
            def insert_values_to_ui(current_config):
                return {
                    pretrained_prior_path: current_config["paths"][
                        "pretrained_prior_path"
                    ],
                    ...
                    text_encoder_path: current_config["paths"]["text_encoder_path"],
                    tokenizer_path: current_config["paths"]["tokenizer_path"],
            ...
            def update_config_from_ui(params):
                ...
                updated_config["paths"]["pretrained_prior_path"] = params[
                    pretrained_prior_path
                ]
                ...
                updated_config["paths"]["text_encoder_path"] = params[text_encoder_path]
                updated_config["paths"]["tokenizer_path"] = params[tokenizer_path]
            ...
            def save_config_values(path, current_config):
                if os.path.exists(path):
                    print("existing lora prior config file found, overwriting")
        ...
        with gr.Column(scale=1):
            ...
            start_lora_prior_training = gr.Button(
                "Start LoRA prior training", variant="primary"
            )
            lora_prior_training_info = gr.HTML(
                "Training not started", elem_classes=["lora-prior-progress"]
            )
            ...
            def launch_training(success, lora_training_config, progress=gr.Progress()):
                ...
                launch_lora_prior_training(kubin, lora_training_config, progress)
                print("\ntraining of LoRA prior model completed")
            ...
            start_lora_prior_training.click(
                ...
                outputs=[ready_to_train, lora_prior_training_info],
                queue=False,
                show_progress=False,
            ).then(
                fn=launch_training,
                inputs=[ready_to_train, training_config],
                outputs=[lora_prior_training_info],
                queue=True,
            ).then(
            ...
            with gr.Accordion("Miscellaneous", open=True) as misc_params:
                with gr.Row():
                    config_path = gr.Textbox(
                        "train/train_lora_prior_config.yaml", label="Config path"
                    )
            ...
            reset_config.click(
                ...
                inputs=[gr.State(default_lora_prior_config_path), current_lora_config],
                outputs=[current_lora_config, config_error],
                queue=False,
            ).then(
