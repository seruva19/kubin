...
from train_modules.train_prior import default_prior_config_path, add_default_values, start_prior_training
  ...
  with gr.Row() as train_prior_block:
    ...
    with gr.Column(scale=3):
      with gr.Accordion('General params', open=True):
        with gr.Row():
          ...
          clip_mean_std_path = gr.Textbox(value=default_config_from_path['clip_mean_std_path'], label='Clip mean standard path', interactive=True, info=relative_path_app_warning) # type: ignore
        ...
        with gr.Row():
          with gr.Column(scale=2):
            with gr.Row():
              ...
              num_workers = gr.Number(value=default_config_from_path['data']['train']['num_workers'], label='Number of workers', info='Set to 0 if Windows', interactive=True) # type: ignore" interactive=True) # type: ignore
      ...
      with gr.Accordion('Miscellaneous', open=True):
        with gr.Row():
          config_path = gr.Textbox('train/train_prior_config.yaml', label='Config path', info=relative_path_extension_warning)
      ...
      reset_config.click(
        fn=load_config_values_from_path, inputs=[gr.State(default_prior_config_path), current_config], outputs=[current_config, config_error], queue=False).then(
    ...
    with gr.Column(scale=1):
      ...
      start_training.click(
        ...
        fn=check_training_params, inputs=[training_config], outputs=[ready_to_train, prior_training_info], queue=False, show_progress=False).then(
        fn=launch_training, inputs=[ready_to_train, dir_root, training_config], outputs=[prior_training_info])
