...
from train_modules.train_prior import relative_path_app_warning

def train_tools_ui(kubin):
  with gr.Accordion(open=True, label='Create dataset'):
    with gr.Row():
      with gr.Column():
        image_folder_path = gr.Textbox('train/images', label='Path to image folder', info=relative_path_app_warning)
        image_extensions = gr.CheckboxGroup(['.jpg', '.jpeg', '.png', '.bmp'], value=['.jpg', '.jpeg', '.png', '.bmp'], label='Image files')
      with gr.Column():
        resize_images = gr.Checkbox(label='Resize images', default=False)
        resized_images_path = gr.Textbox('train/images_resized', label='Path to folder with resize images', info=relative_path_app_warning)
    with gr.Row():
      caption_extension = gr.Textbox('.txt', label='Caption files extension')
      output_csv_path = gr.Textbox('train/dataset.csv', label='Path to output dataset file', info=relative_path_app_warning)
    with gr.Row():
      create_df = gr.Button('Create dataset', variant='primary')
      view_df = gr.Button('View dataset')
      clear_existing = gr.Button('Clear existing dataset and resized images').style(size='sm')
    ...
    create_df.click(lambda: 'Preparing dataset...', outputs=[dataframe_result], queue=False, show_progress=False).then(
      fn=prepare_dataset,
        inputs=[image_folder_path, gr.State(kubin.root), image_extensions, caption_extension, output_csv_path, resize_images, resized_images_path],
        outputs=[dataframe_result, dataframe_error], queue=False, show_progress=False, # type: ignore
      ).then(fn=None, inputs=[dataframe_error], outputs=[dataframe_error], show_progress=False, _js='(e) => !e ? kubin.notify.success("Dataset created") : kubin.notify.error("Error creating dataset")')
  
    clear_existing.click(
      fn=clear_existing_data,
      inputs=[gr.State(kubin.root), output_csv_path, resize_images, resized_images_path],
    ...
    ).then(fn=None, inputs=[dataframe_not_exists, dataframe_result], outputs=[dataframe_not_exists], _js='(err, res) => err && kubin.notify.error(res)')

def load_dataframe(root_path, csv_path):
  csv_path = csv_path if os.path.isabs(csv_path) else os.path.join(root_path, csv_path)
...
def clear_existing_data(root_path, csv_path, resize_enabled, resized_path):
  csv_path = csv_path if os.path.isabs(csv_path) else os.path.join(root_path, csv_path)
  resized_path = resized_path if os.path.isabs(resized_path) else os.path.join(root_path, resized_path)
  ...
  return gr.update(visible=False), gr.update(visible=False), 'Existing output data was cleared'

def prepare_dataset(image_dir, root_path, image_extensions, caption_extension, csv_path, resize_enabled, resized_path):
  data = []

  image_dir = image_dir if os.path.isabs(image_dir) else os.path.join(root_path, image_dir)
  csv_path = csv_path if os.path.isabs(csv_path) else os.path.join(root_path, csv_path)
  resized_path = resized_path if os.path.isabs(resized_path) else os.path.join(root_path, resized_path)
  ...
  if resize_enabled:
    ...
    if len(os.listdir(resized_path)) == 0:
      for image_path, caption_text in data:
        ...
        new_image_path = os.path.join(resized_path, os.path.basename(image_path))
