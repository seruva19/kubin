...
def quantize_with_torch_ao(model, freeze=False, save_name=None):
    ...
    initial_size = sum(p.numel() * p.element_size() for p in initial_params) / (1024**2)  # MB

    # Apply quantization (modifies in-place)
    ...
    # Calculate actual storage size (not dequantized size)
    actual_storage_size = 0
    int8_params_found = 0
    for p in quantized_params:
        if hasattr(p, 'tensor_impl') and hasattr(p.tensor_impl, 'data'):
            # This is a quantized tensor - use actual int8 storage size
            impl_data = p.tensor_impl.data
            actual_storage_size += impl_data.numel() * impl_data.element_size()
            if impl_data.dtype == torch.int8:
                int8_params_found += 1
        else:
            # Regular tensor (like bias)
            actual_storage_size += p.numel() * p.element_size()
    actual_storage_size /= (1024**2)  # Convert to MB

    # Check if model structure changed (quantization adds wrapper modules)
    has_quantized_tensors = any(
        'LinearActivationQuantizedTensor' in str(type(p)) or
        'AffineQuantizedTensor' in str(type(p))
        for p in model.parameters()
    )

    print(f"   → Before: {initial_dtype}, {initial_size:.1f} MB")
    print(f"   → After:  {quantized_dtype} (wrapper), actual storage: {actual_storage_size:.1f} MB")

    if has_quantized_tensors and int8_params_found > 0:
        reduction_pct = (1 - actual_storage_size/initial_size)*100
        print(f"   ✓ Quantization verified: Found {int8_params_found} int8 quantized parameters")
        print(f"   ✓ Memory savings: {initial_size - actual_storage_size:.1f} MB ({reduction_pct:.1f}% reduction)")
    elif has_quantized_tensors:
        print(f"   ⚠️  WARNING: Quantized tensor structure found but no int8 parameters detected")
    ...
    if save_name is not None:
        k_log("⚠️  Cannot (yet) save quantized weights with torch_ao - feature not implemented")
    return model
