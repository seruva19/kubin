    ...
    with gr.Column() as extensions_block:
        ...
        for index, extension_info in enumerate(extensions_data):
            with gr.Column():
                ...
                with gr.Row():
                    gr.Textbox(
                        f'{extension_info["title"]}',
                        label="Title",
                        lines=1,
                        interactive=False,
                    )
                    gr.Textbox(
                        f'{extension_info["path"]}',
                        label="Path",
                        lines=1,
                        interactive=False,
                    )
                with gr.Row():
                    gr.Textbox(
                        f'{extension_info["is_tab"]}',
                        label="Exposes tab",
                        lines=1,
                        interactive=False,
                    )
                    gr.Textbox(
                        f'{extension_info["is_augment"]}',
                        label="Exposes augmentation",
                        lines=1,
                        interactive=False,
                    )
                with gr.Row():
                    gr.Textbox(
                        f'{extension_info["url"]}',
                        label="URL",
                        lines=1,
                        interactive=False,
                    )
                    gr.Textbox(
                        f'{"enabled" if extension_info["enabled"] else "disabled"}',
                        label="Enabled",
                        lines=1,
                        interactive=False,
                    )
                ...
                with gr.Row():
                    with gr.Column(scale=1):
                        clear_ext_install_btn = gr.Button(
                            value="Force reinstall on next launch",
